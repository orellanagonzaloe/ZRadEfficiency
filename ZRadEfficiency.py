...
def loop(inputFiles, year):
    ...
    for syst in cfg['syst_list']:
        ...
        d_den_syst[syst] = ROOT.TH2D('h_den_syst_' + syst, 'h_den_syst_' + syst, len(cfg['et_binning']) - 1,
                                     array('f', cfg['et_binning']), len(cfg['eta_binning']) - 1,
                                     array('f', cfg['eta_binning']))
        d_den_syst[syst].Sumw2()

        for tr in cfg['trigger_list'][year]:
            d_num_syst_tr[syst][tr] = ROOT.TH2D('h_num_syst_tr_' + syst + '_' + tr, 'h_num_syst_tr_' + syst + '_' + tr,
                                                len(cfg['et_binning']) - 1, array('f', cfg['et_binning']),
                                                len(cfg['eta_binning']) - 1, array('f', cfg['eta_binning']))
    ...
    for entry in xrange(nevents):
        ...
        l1_ID = [getattr(chain, 'l1.medium_id'), getattr(chain, 'l1.tight_id')]
        # l1_ID.append(getattr(chain, 'l1.IDloose')) # not used

        l2_ID = [getattr(chain, 'l2.medium_id'), getattr(chain, 'l2.tight_id')]
        # l2_ID.append(getattr(chain, 'l2.IDloose')) # not used

        l1_ISO = [getattr(chain, 'l1.isoloose'), getattr(chain, 'l1.isotight')]

        l2_ISO = [getattr(chain, 'l2.isoloose'), getattr(chain, 'l2.isotight')]

        for s in cfg['syst_list']:

            if llg_m > cfg['llg_m_Min'][cfg['syst_list'][s][0]] and llg_m < cfg['llg_m_Max'][cfg['syst_list'][s][1]] and \
                    cfg['ll_m_Min'][cfg['syst_list'][s][2]] < ll_m < cfg['ll_m_Max'][
                cfg['syst_list'][s][3]] and \
                    l1_ID[cfg['syst_list'][s][5]] and l1_ISO[cfg['syst_list'][s][4]] and l2_ID[
                cfg['syst_list'][s][5]] and l2_ISO[cfg['syst_list'][s][4]] and \
...
def createEff(year, sam):
    ...
    for syst in cfg['syst_list']:
        ...
        for tr in cfg['trigger_list'][year]:
            ...
            h_eff_tr[syst][tr] = ROOT.TH2D('h_eff_syst_tr_' + syst + '_' + tr, 'h_eff_syst_tr_' + syst + '_' + tr,
                                           len(cfg['et_binning']) - 1, array('f', cfg['et_binning']),
                                           len(cfg['eta_binning']) - 1, array('f', cfg['eta_binning']))
            ...
            for et in xrange(1, len(cfg['et_binning'])):
                for eta in xrange(1, len(cfg['eta_binning'])):
                    ...
                    h_eff_tr[syst][tr].SetBinError(et, eta, eff_tmp.GetErrorYlow(
                        0))  # TH2 cannot store asymmetric errors, so we only use the lower error considering is always the greater
...
def createSF(year):
    for tr in cfg['trigger_list'][year]:
        ...
        h_SF = ROOT.TH2D('h_SF', 'h_SF', len(cfg['et_binning']) - 1, array('f', cfg['et_binning']),
                         len(cfg['eta_binning']) - 1, array('f', cfg['eta_binning']))
        h_SF.Sumw2()
...
def print_progressbar(name, total, progress):
    ...
    text = '\rProcessing {:2}  [{}] {:.0f}% {}'.format(name, '#' * block + '-' * (bar_length - block),
...
if args.loop:
    loop(args.inputFiles, args.year)
elif args.createSF:
    SFs()
